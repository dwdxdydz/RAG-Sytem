...
st.caption("Upload PDFs, retrieve evidence, and optionally generate an answer from the retrieved context.")
...
if files:
    for uploaded in files:
        if uploaded.name in {c.source for c in store.chunks}:
            continue
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            tmp.write(uploaded.getbuffer())
            tmp_path = Path(tmp.name)
        count = store.add_pdf(str(tmp_path))
